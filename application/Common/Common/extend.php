 ...
 * @param bool $back	是否跳转前一页,默认跳转前一页
 * @param string $local	跳转地址，默认值为空
 * @param bool $exit	服务端是否停止运行，默认停止运行
 */
function alert($str,$back=true,$local='',$exit=true){
	$str="<script>alert('$str');";
	if($back)$str.="history.back();";
	if(!empty($local))$str.="location='$local';";
	$str.="</script>";
	echo $str;
	if($exit)exit;	
}
